...
interface SettingsModalProps {
  ...
  initialTab?: 'themes' | 'fonts' | 'general';
  className?: string;
  embedded?: boolean;
}
...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'themes' | 'fonts' | 'general'>(initialTab);
  ...
  const content = (
    <div className={clsx(
      "bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden border border-gray-200 dark:border-gray-700 flex flex-col h-[750px]",
      embedded && "max-w-none h-full shadow-none border-none rounded-none",
      className
    )}>
      {/* Header */}
